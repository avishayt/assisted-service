...
func (o *DeleteImageURL) Build() (*url.URL, error) {
	...
	var _path = "/clusters/{cluster_id}/images/{image_id}"

	clusterID := o.ClusterID.String()
	if clusterID != "" {
		_path = strings.Replace(_path, "{cluster_id}", clusterID, -1)
	} else {
		return nil, errors.New("clusterId is required on DeleteImageURL")
	}

	imageID := o.ImageID.String()
	if imageID != "" {
		_path = strings.Replace(_path, "{image_id}", imageID, -1)
	} else {
		return nil, errors.New("imageId is required on DeleteImageURL")
	}
