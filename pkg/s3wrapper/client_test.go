...
var _ = Describe("s3client", func() {
	...
	BeforeEach(func() {
		...
		client = &S3Client{log: log, session: nil, client: mockAPI, publicClient: publicMockAPI, cfg: &cfg, isoUploader: isoUploader}
	...
	It("download_boot_file", func() {
		...
		_, name, length, err := client.DownloadBootFile(ctx, RHCOSBaseObjectName, "vmlinuz")
	...
	It("get_s3_boot_file_url", func() {
		...
		url := client1.GetS3BootFileURL(RHCOSBaseObjectName, "rootfs.img")
		...
		url = client2.GetS3BootFileURL(RHCOSBaseObjectName, "initrd.img")
