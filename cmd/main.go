...
func main() {
	...
	case deployment_type_onprem, deployment_type_ocp:
		...
		err = objectHandler.UploadBootFiles(context.Background(), s3wrapper.RHCOSBaseObjectName, s3wrapper.RHCOSBaseURL)
		if err != nil {
			log.WithError(err).Fatalf("Failed to extract and upload files from ISO %s", s3wrapper.RHCOSBaseObjectName)
		}
	default:
	...
	case deployment_type_k8s:
		go func() {
			...
			failOnError(baseISOUploadLeader.RunWithLeader(context.Background(), func() error {
				return objectHandler.UploadBootFiles(context.Background(), s3wrapper.RHCOSBaseObjectName, s3wrapper.RHCOSBaseURL)
